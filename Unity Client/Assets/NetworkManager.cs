...
public class NetworkManager : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject networkGameObjectInstance;
    ...
    void Update()
    {
        while (udpClient.Available > 0)
        {
            ...
            if (p is TransformPacket t)
            {
                ...
                GameObject found = networkObjects.Find(obj => obj.GetComponent<NetworkGameObject>().networkID == t.entity);
                if (found != null) 
                {
                    found.transform.position = t.getPos();
                    found.transform.rotation = t.getRot();
                }
                else
                {
                    var newObj = Instantiate<GameObject>(networkGameObjectInstance);
                    newObj.GetComponent<NetworkGameObject>().networkID = t.entity;
